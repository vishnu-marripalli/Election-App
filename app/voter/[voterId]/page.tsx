...
>({});
  ...
  const fetchVoter = async () => {
    try {
      const res = await fetch(`/api/user/${voterId}`);
      if (res.ok) {
  ...
  const handleVote = async (candidateId: string, electionId: string, position: string) => {
    ...
    try {
      const res = await fetch('/api/votes', {
        ...
        body: JSON.stringify({voterId: voterData?._id, candidateId, electionId, position })
      });
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 text-gray-600 mb-1">
                  ...
                  <span className="font-medium">Voting Dashboard</span>
                </div>
            ...
            <div className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-6 py-4 rounded-xl shadow-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="font-bold">{voterData?.studentId}</div>
                </div>
              </div>
            </div>
          </div>
